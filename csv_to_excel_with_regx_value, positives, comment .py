...
output_file = f'excel_files/file_output_{date_suffix}_with_regx_value_positives_comments.xlsx'
...
def process_excel(input_file, output_file):
    ...
    # Add the new 'Positives' and 'Comment' columns at the end
    df['Positives'] = ''
    df['Comment'] = ''
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        worksheet.write(1, 0, '', subheader_format)  # Empty subheader under Tag

        # Prepare headers for merging
        header_names = df.columns.tolist()
        header_top = [col.split()[0] for col in header_names]
        header_sub = [col.split()[1] if len(col.split()) > 1 else '' for col in header_names]
        ...
        for col_num, (top, sub) in enumerate(zip(header_top, header_sub), start=1):
            if top != current_top:
                # Merge the previous group if it exists
                if current_top is not None:
                    # If we have only one column in the previous group, just write the cell without merging.
                    if start_col == col_num - 1:
                        worksheet.write(0, start_col, current_top, header_format)
                    else:
        ...
        # Merge the final group if exists
        if current_top is not None:
            # If the final group also spans a single column
            if start_col == len(header_top) + 1 - 1:  # The current group is just one column
                worksheet.write(0, start_col, current_top, header_format)
            else:
        ...
        # Apply conditional formatting for Before/After comparison
        # Identify metric columns: each metric has 3 columns (Before, After, RegX)
        metric_names = ['Precision', 'Recall', 'F1']
        metric_col_indices = []
        for i, col in enumerate(header_names):
            if col.split()[0] in metric_names:
                metric_col_indices.append(i)

        # Apply conditions in groups of three columns (Before, After, RegX)
        for start in range(0, len(metric_col_indices), 3):
            before_col = 1 + start     # Excel column index (since Tag is at col 0)
            after_col = before_col + 1
            regx_col = before_col + 2
